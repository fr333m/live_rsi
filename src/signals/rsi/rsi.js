...
async function calculationRSI(interval) {
    ...
    const uniqueSymbols = await dbService.uniqueSymbol(
    ...
    for (const symbol of uniqueSymbols) {
        ...
        const ohlc = await dbService.getCandles(
            symbol,
            interval,
            'tracking_contracts'
        );
        ...
        console.log(
            `calculationRSI() received ${uniqueSymbols.length} symbols for RSI analysis.`
        );
        ...
        if (rsiValue === null) {
            console.log(
                `Symbol: ${symbol}, RSI: Not enough data, interval: ${interval}`
            );
            continue;
        }

        const actualSignal = await checkActualSignal(
            symbol,
            interval,
            currentTimestamp,
            typeSignal,
        ...
        if (rsiValue > 65) {
            ...
            try {
                ...
                console.error(
                    `Failed to send signal for ${symbol}:`,
                    err.message
                );
            }
        }
        ...
        console.log(
            `Symbol: ${symbol}, RSI: ${rsiValue}, interval: ${interval}`
        );
    }
